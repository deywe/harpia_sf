# -*- coding: utf-8 -*-
# ───────────────────────────────────────────────────────────────
# File: sphy_sf_qaoa_v02.3_EN.py (ROBUST VERSION WITH NUMERICAL HANDLING)
...
# === Logging/Warnings Configuration (Minimized)
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3' 
logging.getLogger('tensorflow').setLevel(logging.FATAL)
os.environ["CUDA_VISIBLE_DEVICES"] = "-1" 
# Note: Error handling in compute_zz_expectation should be sufficient, 
# but silencing can be reactivated if necessary.
...
try:
    # Modules must be available in the local environment
...
# === ROBUST PURITY CALCULATION FUNCTION
def compute_zz_expectation(state):
    """
    Calculates Purity (as a coherence proxy) robustly.
    Handles numerical errors that cause RuntimeWarnings (det < 0 or det = 0).
    """
    V = state.cov()
    ...
    try:
        ...
        # If the matrix is singular and det fails (rare but possible)
        return 1e6 
        
    # If the determinant is non-positive (causes RuntimeWarning: invalid value encountered in sqrt)
    if det_2V <= 0:
        # Returns a very bad energy (minimization avoids this point)
        return 1e5 
    
    # If det_2V is very close to zero (causes RuntimeWarning: divide by zero)
    if det_2V < 1e-18:
        return 1e5 

    purity = 1.0 / np.sqrt(det_2V)
    
    # Returns Negative Purity (so that energy is minimized)
    return -purity
...
def attempt_controlled_tunneling_sf(valid_time, round_id, psi0_trace_str, f_opt):
    ...
    # Optimization. Thanks to the error handling in the cost function, it will be more robust.
    res = minimize(compute_energy_sf, init_params, method='Powell')
    energy = res.fun 
    
    # Checks if optimization resulted in an invalid value (from our error handling)
    if energy >= 1e5:
...
def run_harpia_round(args):
    ...
    # calcular_F_opt is now F_opt_calculator
...
# ====================================================================
# === PLOTTING FUNCTIONS (FINAL)
# ====================================================================

def generate_plots_from_log(log_file, timestamp_id):
    """
    Generates the 2D Plot of Energy vs. Rounds and the 3D Plot of the SPHY Trajectory.
    """
    ...
    try:
        ...
        # --- 1. 2D Plot: Energy (Purity) vs. Rounds ---
        ...
        # Correction to ensure 'energy' is float
        ...
        plt.xlabel('Round (Tunneling Attempt)')
        ...
        # --- 2. 3D Plot: SPHY Trajectory (Last Accepted Round) ---
        ...
        ax.set_xlabel('Px (Symmetric Noise)')
        ax.set_ylabel('Py (Asymmetric Noise)')
        ax.set_zlabel('Pz (Phase Noise)')
